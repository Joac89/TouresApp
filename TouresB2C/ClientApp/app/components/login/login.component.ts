...
export class LoginComponent {
    ...
    constructor(@Inject('BASE_URL') baseUrl: string, private router: Router, private http: Http, private tokenService: TokenService, private authService: AuthService, private loaderService: LoaderService) {
        this.path = baseUrl;
    }
    ...
    sendLogin() {
        ...
        this.http.post(this.path + "api/customer/login", json, token).map(response => response.json()).subscribe(result => {
